...
let buildingDetailsById = new Map();
...
function scheduleClusterLayerUpdate() {
...
async function loadData(url, retries = 1, useCache = true) {
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      try {
        ...
        if (useCache) dataCache.set(url, parsed);
        return parsed;
      } catch (jsonError) {
...
function initAnalyticsCharts() {
  ...
  if (transportCtx) {
    const existingChart = Chart.getChart('transport-chart');
    if (existingChart) {
  ...
  if (educationCtx) {
    const existingChart = Chart.getChart('education-chart');
    if (existingChart) {
       existingChart.destroy();
    }
  ...
  if (healthcareCtx) {
    const existingChart = Chart.getChart('healthcare-chart');
    if (existingChart) {
       existingChart.destroy();
    }
  ...
  if (housingCtx) {
    const existingChart = Chart.getChart('housing-chart');
    if (existingChart) {
       existingChart.destroy();
    }
  ...
  try {
    ...
    if (buildingsData && buildingsData.buildings) {
      allFeaturesByType.building = Object.values(buildingsData.buildings).map((building) => {
        ...
        feature.set("socialScore", building[15]);
        feature.set("qualityScore", building[16]);
        feature.set("transportScore", building[17]);
        feature.set("totalScore", building[18]);
        return feature;
      });
      buildingDetailsById = new Map(
        Object.values(buildingsData.buildings).map((row) => [String(row[0]), row])
      );
    }
    ...
    [
      ...
      if (checkbox) {
        checkbox.addEventListener("change", scheduleClusterLayerUpdate);
      }
    });
    ...
    if (map) {
      map.on("click", (e) => {
        ...
        if (clusterFeatures.length > 1) {
          ...
          if (type === "building") {
            showBuildingDetails(feature, e.originalEvent);
          } else if (type === "school") {
...
function showBuildingDetails(feature, event) {
  ...
  try {
    ...
    const arr = buildingDetailsById.get(String(buildingId));
    if (arr) {
      [socialScore, qualityScore, transportScore, totalScore] = arr.slice(15, 19);
      lat = parseFloat(arr[11]).toFixed(6);
      lon = parseFloat(arr[12]).toFixed(6);
      floors = arr[4] ?? floors;
      build_year = arr[3] ?? build_year;
      is_emergency = arr[5] ? "Да" : "Нет";
    }
  }
  catch (error) {
